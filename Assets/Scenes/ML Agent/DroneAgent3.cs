...
public class DroneAgent3 : Agent
{
    ...
    private Vector3 GoalPosition1, GoalPosition2, currentGoal;
    ...
    public override void Initialize()
    {
        ...
        GoalPosition1 = new Vector3(goal.position.x, goal.position.y, goal.position.z);

        GoalPosition2 = new Vector3(goal.position.x - 7f, goal.position.y + 8f, goal.position.z - 7f);
        ...
        logFilePath = Path.Combine(folder, "drone_positions_runid2_test3.csv");
    ...
    {   
        ...
        Vector3 toGoal = GoalPosition1 - position;
        ...
        // sensor.AddObservation(GoalPosition1.x);
        // sensor.AddObservation(GoalPosition1.y);
        // sensor.AddObservation(GoalPosition1.z);
        ...
        // float distanceToGoal = Vector3.Distance(BaseLink.transform.position, GoalPosition1);
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        Vector3 currentGoal = (currentGoalIndex == 0) ? GoalPosition1 : GoalPosition2;
        float distanceToGoal = Vector3.Distance(BaseLink.transform.position, currentGoal);
        ...
        if (debugMode) Debug.Log($"[DroneAgent] Goal Index: {currentGoalIndex} Distance to Goal: {distanceToGoal} | Reward: {reward}");
    ...
    public void FixedUpdate()
    {
        ...
        if (distanceToGoal < 1f)
        {
            ...
            {
                Debug.Log($"EPISODE ENDED, *******GOAL 1 REACHED*******, Distance to Goal:  {distanceToGoal}");
                currentGoalIndex++;
            }
            else if (currentGoalIndex == 1)
            {
                Debug.Log($"EPISODE ENDED, **********************GOAL 2 REACHED**********************, Distance to Goal:  {distanceToGoal}");
                EndEpisode();
            }
        }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (debugMode) Debug.Log($" [ResetAgent] Goal Position  : {currentGoal.x}, {currentGoal.y}, {currentGoal.z}");
    ...
    void WritePositionsToFile()
    {
        using (StreamWriter writer = new StreamWriter(logFilePath, true)) // true to append
        {
            ...
            {
                writer.WriteLine($"{episodeCounter},{pos.x},{pos.y},{pos.z}");
            }
    ...
    {   
        ...
        var NewPosition = ENU.ConvertToRUF(
            new Vector3(
                (float)initialPositionSAM[0]+5f + Random.Range(-2f,2f),  
